class units():
        ...
        for type in range(len(unit_types)):
            ...
            for flip in range(2):
                for i in range(unit_types[type][1]):
                    ...
                    self.images_shadows[0][unit_types[type][0]][flip].append(PIL_to_pyglet(image_transform_for_shadow(self.images[0][unit_types[type][0]][flip][i], SHADOWS_COLOR, True), SCALE_WORLD/1.5))

                    self.images[0][unit_types[type][0]][flip][i] = PIL_to_pyglet(self.images[0][unit_types[type][0]][flip][i], SCALE_WORLD/1.5)
    ...
    def add_unit(self, type=0, flip=0):
        ...
        if type == 0:
            ...
            self.unit_list[flip].append([unit(0, y, self.images[0]['run'][not flip], self.images_shadows[0]['run'][not flip], self.units_info[country][type], 0, flip), y])
